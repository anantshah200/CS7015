...
\begin{Solution}
The transformation matrix is such that it transforms all $3-d$ vectors into a $2-d$ plane, hence giving a volume of 0 for each transformed volume. The columns of the matrix are not linearly independent($|A|=0$) and hence they do not form a valid basis. 
\end{Solution}
\end{parts}
...
Thus the constants $\alpha_{i}$ can be found by taking the inner product of $x$ with each orthonormal vector.
\[\alpha_{i} = \frac{\inp{x}{v_{i}}}{\norm{v_{i}}^{2}}\]
where $\norm{v_{i}}^2=1$ and the inner product is defined as
\[\inp{x}{y} = x^{T}y\]
\end{Solution}
\end{parts}
...
\begin{parts}
    ...
    Hence the orthonormal basis obtained using Gram-Schmidt orthogonalization technique is  $
        \langle \frac{1}{\sqrt{3}}\left(\begin{array}{cc}
         1 \\ 1 \\ 1
...
\begin{parts}
    ...
    On solving this system on linear equations, we can get our variable values. However there are $2$ equations and $4$ variables, which leads to infinite solutions which will be non-zero. Hence any set of 4 unique vectors in ${\rm I\!R^{2}}$ are linearly dependent.
    \end{Solution}
